...
def extract_rbd_saliency_folder(folder_path, output_root):
    ...
    guarantee_path_preconditions(output_root)
    ...
    with ProcessPoolExecutor() as executor:
        for frame in files:
            ...
            if not os.path.exists(output_path):
                executor.submit(extract_saliency, path_frame, output_path)


def extract_saliency(file_path, output_path):
    print('Extracting saliency from: ' + file_path)
    print('Extracting saliency to: ' + output_path)
    if not os.path.exists(output_path):
        try:
...
if __name__ == '__main__':
    ...
    extract_rbd_saliency_folder(path_in, path_out)